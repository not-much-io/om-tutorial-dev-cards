...
(defui WidgetWithProperties
  ...
  (render [this]
    (let [{:keys [name age]} (om/props this)]
      (dom/div nil (str "Hello " name " age: " age)))))
...
(defcard props-card (prop-widget {:name "Sam" :age 20}))
...
(defui Person
  ...
  (render [this]
    (let [{:keys [name age]} (om/props this)]
      (dom/li nil
              (dom/div nil
                       (dom/span (clj->js {:style {:color "red"}}) name)
                       (dom/span nil age))))))
...
(defcard root-render (root {:number 42 :people [{:name "Sam" :age 20} {:name "Joe" :age "?"}]}))
...
(defcard passing-callbacks-via-computed
         (fn [data-atom-from-devcards _]
           (let [prop-data @data-atom-from-devcards
                 sideband-data {:incHandler  (fn [] (swap! data-atom-from-devcards update-in [:number] #(+ % 2)))
